...
describe('edit_file tool', () => {
  ...
  it('replaces a single occurrence', async () => {
    ...
    expect(result.content).toBe(`Replaced 1 occurrence(s) in ${filePath}`)
    expect(result.displayContent).toContain('diff --git')
    expect(result.displayContent).toContain('--- a/')
    expect(result.displayContent).toContain('+++ b/')
    expect(result.displayContent).toContain('-const x = 1')
    expect(result.displayContent).toContain('+const x = 42')
    expect(result.displayContent).not.toContain('Replaced 1 occurrence')
